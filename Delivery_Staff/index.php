...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['order_id']) && isset($_POST['action'])) {
    ...
    if ($action == 'shipped') {
        if ($current_status != 'shipped') {
            $sql = "UPDATE orders_table SET status = 'shipped' WHERE order_id = $order_id";
...
WHERE status = 'confirmed' ORDER BY orders_table.created_at DESC";
...
<div class="confirmorder">
    <h3 class="conf-order">Confirmed Orders are here:-</h3>
    <div class="orderlist">
        <table style="width:70%;" border="2">
            ...
            <tr>
                ...
                <td>
                   <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" name="shipped">
                   <input type="hidden" name="order_id" value="<?php echo $row['order_id']?>">
                   <input type="hidden" name="action" value="shipped">
                   <button type="submit" <?php if($row['status'] == 'shipped') echo 'disabled';?> class=" delivery">Shipped</button>
                   </form>
                </td>
            </tr>
            <?php
